...
#+BEGIN_SRC sh
  ...
  sudo apt-get install build-essential automake texinfo libjpeg-dev libncurses5-dev libtiff5-dev libgif-dev libpng-dev libxpm-dev libgnutls28-dev libmagick++-dev libgtk-3-dev
  cd ~/src/emacs
  ./autogen.sh
...
Si bien en este punto ya se instaló emacs, es necesario instalar el
soporte para el cliente de correo, para ello es necesario realizar los
siguientes pasos:

+ Instalar =offlineimap= para poder conectarnos a un servidor de correo:

  #+begin_src sh
    sudo apt install offlineimap guile-2.0-dev html2text xdg-utils
    libgmime-3.0-dev libxapian-dev
  #+end_src

  Crear el archivo =~/.offlineimaprc= que contendrá los parámetros de conexión.
  ...
  #+begin_src sh
    ...
    remoteuser = usuer@prepaenlinea.mx
  ...
  Con la configuración establecida, es momento de probar la
  conectividad con el servidor (esto puede tardar unos minutos,
  dependiendo del número de mensajes que tenga la cuenta):

  #+begin_src sh
    offlineimap
  #+end_src

  Debemos esperar a que aparezca un mensaje similar a =***** Finished
  processing account Gmail= Con la configuración correcta podemos
  proceder a instalar =mu=
...
Por último se deben indexar los mensajes:
