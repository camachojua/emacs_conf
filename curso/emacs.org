...
#+begin_src sh
  cd ~/src/emacs
  git pull
  ./configure --enable-autodepend --with-libsystemd --without-modules --with-x-toolkit=gtk --with-mailutils --with-imagemagick --with-jpeg --with-tiff --with-gif --with-png --with-rsvg --with-libotf --with-xaw3d --with-gpm --with-json
  make -j$(nproc) -s bootstrap
  sudo make -j$(nproc) -s install
#+end_src
